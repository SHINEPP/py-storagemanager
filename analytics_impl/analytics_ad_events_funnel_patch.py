...
if __name__ == '__main__':
    ...
    sql = f'''
SELECT event_date_utc, json_extract(event_parameters, '$.security_patch') as security_patch, event_name, count(distinct user_id) as user_count, count(*) as event_count
FROM `macrophage_data_{app_name}`.raw_data
WHERE {where}
GROUP BY event_date_utc, security_patch, event_name
...
LIMIT 0, 2000
    ...
    with open_analytics() as cursor:
        ...
        headers = ['event_date_utc', 'security_patch', 'event', 'user_count', 'event_count', 'per_user_event_count']
        csv_writer.writerow(headers)

        for row in rows:
            csv_writer.writerow([row[0], row[1], row[2], row[3], row[4], round(row[4] / row[3], 1)])
        csv_file.close()
